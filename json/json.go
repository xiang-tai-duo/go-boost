// Package json
// File:        json.go
// Url:         https://github.com/xiang-tai-duo/go-bootstrap/blob/master/json/json.go
// Author:      TRAE.AI
	...
	if len(json) > 0 {
		...
		if s, ok := json[0].(string); ok {
			...
			if err := jsonlib.Unmarshal([]byte(s), &parsedValue); err == nil {
				j.text = s
			} else if jsonBytes, err := jsonlib.Marshal(json[0]); err == nil {
...
func (j *JSON) Format(indent string) (string, error) {
	...
	if err = jsonlib.Unmarshal([]byte(j.text), &parsedValue); err == nil {
		if formatted, err = jsonlib.MarshalIndent(parsedValue, "", indent); err == nil {
...
func (j *JSON) Unmarshal(target interface{}) error {
	return jsonlib.Unmarshal([]byte(j.text), target)
}
...
func (j *JSON) WriteFile(filePath string, indent ...string) error {
	...
	if indentValue != "" {
		if jsonBytes, err = jsonlib.MarshalIndent(j.value, "", indentValue); err == nil {
			// No return here, continue
		} else {
			return err
		}
	} else {
		if jsonBytes, err = jsonlib.Marshal(j.value); err == nil {
			// No return here, continue
		} else {
			return err
		}
