...
const (
	MODULE_NAME_WINBASE = "windows.winbase"
	NULL_HANDLE         = 0
	WCHAR_SIZE          = 2
)
...
func __debug(message string) {
	logger.Logger.DebugEx(message, MODULE_NAME_WINBASE, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __info(message string) {
	logger.Logger.InfoEx(message, MODULE_NAME_WINBASE, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __warning(message string) {
	logger.Logger.WarningEx(message, MODULE_NAME_WINBASE, logger.SKIP_STACK_FRAMES_BASE)
}
...
func __error(message interface{}) {
	logger.Logger.ErrorEx(message, MODULE_NAME_WINBASE, logger.SKIP_STACK_FRAMES_BASE)
}
