...
const (
	...
	PRINTER_PORT_NAME_IP_PREFIX  = "IP_"
	REGISTRY_TCPIP_PORTS_PATH    = `SYSTEM\CurrentControlSet\Control\Print\Monitors\Standard TCP/IP Port\Ports`
	REGISTRY_VALUE_HOST_NAME     = "HostName"
	REGISTRY_VALUE_IP_ADDRESS    = "IPAddress"
	REGISTRY_VALUE_PROTOCOL      = "Protocol"
	REGISTRY_VALUE_PORT_NUMBER   = "PortNumber"
	PROTOCOL_RAW                 = 1
	PROTOCOL_LPR                 = 2
	MODULE_NAME_WINSPOOL         = "windows.winspool"
)

//goland:noinspection GoUnusedFunction
func __debug(message string) {
	logger.Logger.DebugEx(message, MODULE_NAME_WINSPOOL, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __info(message string) {
	logger.Logger.InfoEx(message, MODULE_NAME_WINSPOOL, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __warning(message string) {
	logger.Logger.WarningEx(message, MODULE_NAME_WINSPOOL, logger.SKIP_STACK_FRAMES_BASE)
}
...
func GetDefaultPrinter() string {
	result := ""
	var cchBuffer C.DWORD
	C.GetDefaultPrinterW(nil, &cchBuffer)
	if cchBuffer > 0 {
		buffer := make([]uint16, cchBuffer)
		if C.GetDefaultPrinterW((*C.wchar_t)(unsafe.Pointer(&buffer[0])), &cchBuffer) != 0 {
...
//goland:noinspection GoUnusedExportedFunction,GoUnhandledErrorResult,GoUnhandledErrorResult
func GetPrinterIP(printerName string) (string, error) {
	result := ""
	err := error(nil)
	portName := ""
	if portName, err = withPrinterPortRegistryKey(printerName, func(portKey registry.Key) error {
		parseValue := func(valueName string) string {
			result := ""
			if result, _, _ = portKey.GetStringValue(valueName); result != "" {
		...
		if result = parseValue(REGISTRY_VALUE_IP_ADDRESS); result == "" {
			result = parseValue(REGISTRY_VALUE_HOST_NAME)
		}
		if result == "" {
			return fmt.Errorf("no valid IPv4 address found in registry (tried IPAddress and HostName)")
		}
		return nil
	}); err == nil {
		__debug(fmt.Sprintf("Printer port [%s] actual IP read from registry: %s", portName, result))
	}
	return result, err
}
...
func GetPrinterPortProtocol(printerName string) (uint32, uint32, error) {
	...
	portName := ""
	if portName, err = withPrinterPortRegistryKey(printerName, func(portKey registry.Key) error {
		protocolVal, _, errVal := portKey.GetIntegerValue(REGISTRY_VALUE_PROTOCOL)
		if errVal == nil {
			protocol = uint32(protocolVal)
			if protocol != PROTOCOL_RAW && protocol != PROTOCOL_LPR {
				return fmt.Errorf("unknown protocol value: 0x%08x", protocol)
			}
		} else {
			return errVal
		}
		if portNumberVal, _, _ := portKey.GetIntegerValue(REGISTRY_VALUE_PORT_NUMBER); portNumberVal > 0 {
			portNumber = uint32(portNumberVal)
		} else {
			portNumber = 0
		}
		return nil
	}); err == nil {
		__debug(fmt.Sprintf("Printer port [%s] protocol read from registry: %d, port number: %d", portName, protocol, portNumber))
	} else {
		protocol = 0
		portNumber = 0
	}
	return protocol, portNumber, err
}
...
func PausePrinter(printerName string) error {
	...
	if IsPrinterExist(printerName) {
		if printerNameW := CString(printerName); printerNameW == nil {
			err = fmt.Errorf("failed to convert printer name to wide string: %s", printerName)
		} else {
			defer FreeCString(printerNameW)
			var hPrinter C.HANDLE
			if C.OpenPrinterW(printerNameW, &hPrinter, nil) != 0 {
				defer C.ClosePrinter(hPrinter)
				if C.SetPrinterW(hPrinter, 0, nil, PRINTER_CONTROL_PAUSE) == 0 {
					err = fmt.Errorf("failed to pause printer: %s", printerName)
				}
			} else {
				err = fmt.Errorf("failed to open printer: %s", printerName)
			}
		}
	} else {
...
func ResumePrinter(printerName string) error {
	...
	if IsPrinterExist(printerName) {
		if printerNameW := CString(printerName); printerNameW == nil {
			...
			if C.OpenPrinterW(printerNameW, &hPrinter, nil) != 0 {
				defer C.ClosePrinter(hPrinter)
				if C.SetPrinterW(hPrinter, 0, nil, PRINTER_CONTROL_RESUME) == 0 {
					err = fmt.Errorf("failed to resume printer: %s", printerName)
				}
			} else {
				err = fmt.Errorf("failed to open printer: %s", printerName)
			}
	...
	if IsPrinterExist(printerName) {
		if printerNameW := CString(printerName); printerNameW == nil {
			...
			if C.SetDefaultPrinterW(printerNameW) == 0 {
				err = fmt.Errorf("failed to set default printer: %s", printerName)
			}
		}
	} else {
		err = fmt.Errorf("printer not found: %s", printerName)
...
func WaitJobByID(printerName string, jobID uint32) error {
	err := error(nil)
	jobInfo := (*JOB_INFO)(nil)
	startTime := time.Now()
	found := false
	for !found {
		if jobInfos := GetJobByID(printerName, jobID); len(jobInfos) > 0 {
			jobInfo = &jobInfos[0]
			found = true
		} else {
			if time.Since(startTime) > time.Duration(JOB_NOT_FOUND_TIMEOUT)*time.Second {
				err = fmt.Errorf("job not found within %d seconds", JOB_NOT_FOUND_TIMEOUT)
				found = true
			} else {
		...
		for !completed {
			if IsJobCompleted(jobInfo.Status) {
				...
				if time.Since(startTime) > time.Duration(JOB_WAIT_TIMEOUT)*time.Second {
					err = fmt.Errorf("job wait timeout after %d seconds", JOB_WAIT_TIMEOUT)
					completed = true
				} else {
...
func __error(message interface{}) {
	logger.Logger.ErrorEx(message, MODULE_NAME_WINSPOOL, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnhandledErrorResult
func withPrinterPortRegistryKey(printerName string, fn func(portKey registry.Key) error) (string, error) {
	err := error(nil)
	portName := ""
	if printerName == "" {
		err = fmt.Errorf("printer name is empty")
	} else {
		printers := GetTcpIpPrinters()
		for _, printer := range printers {
			if printer.Name == printerName {
				portName = printer.PortName
				// Use anonymous function to ensure defer executes at end of each iteration
				err = func() error {
					var key registry.Key
					if key, err = registry.OpenKey(registry.LOCAL_MACHINE, REGISTRY_TCPIP_PORTS_PATH, registry.QUERY_VALUE); err != nil {
						return err
					}
					defer key.Close()
					var portKey registry.Key
					if portKey, err = registry.OpenKey(key, printer.PortName, registry.QUERY_VALUE); err != nil {
						return err
					}
					defer portKey.Close()
					return fn(portKey)
		...
		if portName == "" && err == nil {
			err = fmt.Errorf("printer not found in TCP/IP printer list: %s", printerName)
		}
	}
	return portName, err
}
