// Package sqlite
// File:        sqlite.go
// Url:         https://github.com/xiang-tai-duo/go-bootstrap/blob/master/sqlite/sqlite.go
// Author:      TRAE.AI
...
func (sqlite *SQLITE) BeginTransaction() error {
	...
	if sqlite == nil {
		...
		if _, err = sqlite.sqlDb.Exec("BEGIN TRANSACTION"); err == nil {
...
func (sqlite *SQLITE) Commit() error {
	...
	if sqlite == nil {
		...
		if _, err = sqlite.sqlDb.Exec("COMMIT"); err != nil {
...
func (sqlite *SQLITE) ConvertTextToMD5Text(tableName string, columnName string) error {
	err := error(nil)
	var rows *sql.Rows
	var rowID int64
	var value string
	...
	if sqlite == nil {
		...
		if err = sqlite.BeginTransaction(); err == nil {
			...
			if rows, err = sqlite.sqlDb.Query(query); err == nil {
				defer rows.Close()
				for rows.Next() {
					if err = rows.Scan(&rowID, &value); err == nil {
						if value != "" && !sqlite.isMD5Hash(value) {
							newHashValue = sqlite.MD5(value)
							updateQuery := fmt.Sprintf("UPDATE %s SET %s = ? WHERE rowid = ?", tableName, columnName)
							if _, updateError := sqlite.sqlDb.Exec(updateQuery, newHashValue, rowID); updateError != nil {
								err = fmt.Errorf("failed to update record with row id %d: %w", rowID, updateError)
								break
							}
						}
					} else {
	...
	if sqliteAbsoluteFilePath, err = sqlite.getAbsolutePath(sqliteFilePath); err == nil {
		if _, err = os.Stat(sqliteAbsoluteFilePath); err == nil {
			if sqlite == nil {
				...
				sqlite.Close()
				if sqlite.sqlDb, err = sql.Open("sqlite3", sqliteAbsoluteFilePath); err == nil {
					sqlite.sqlDb.SetMaxOpenConns(1)
					sqlite.sqlDb.SetMaxIdleConns(1)
					sqlite.sqlDb.SetConnMaxLifetime(0)
					var schemaVersion int
					if err = sqlite.sqlDb.QueryRow("PRAGMA schema_version").Scan(&schemaVersion); err != nil {
						sqlite.Close()
			...
			if sqlite.sqlDb, err = sql.Open("sqlite3", sqliteAbsoluteFilePath); err == nil {
				...
				sqlite.sqlDb.SetConnMaxLifetime(0)
				if sqlite.PragmaKey == "" {
					if _, err = sqlite.sqlDb.Exec("PRAGMA synchronous = NORMAL"); err != nil {
						sqlite.Close()
						err = fmt.Errorf("unable to set synchronous mode: %w", err)
						sqlite.handleCGOError(err)
					}
				} else {
					if _, err = sqlite.sqlDb.Exec("PRAGMA key = ?", sqlite.PragmaKey); err == nil {
						var cipherVersion string
						if err = sqlite.sqlDb.QueryRow("PRAGMA cipher_version").Scan(&cipherVersion); err == nil {
							if cipherVersion == "" {
								sqlite.Close()
				...
				if err == nil {
					if _, err = sqlite.sqlDb.Exec("VACUUM"); err == nil {
						sqlite.sqliteFilePath = sqliteAbsoluteFilePath
					} else {
						sqlite.Close()
...
func (sqlite *SQLITE) Rollback() error {
	...
	if sqlite == nil {
		...
		if _, err = sqlite.sqlDb.Exec("ROLLBACK"); err != nil {
