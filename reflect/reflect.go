...
func __debug(message string) {
	logger.Logger.DebugEx(message, MODULE_NAME_REFLECT, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __info(message string) {
	logger.Logger.InfoEx(message, MODULE_NAME_REFLECT, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __warning(message string) {
	logger.Logger.WarningEx(message, MODULE_NAME_REFLECT, logger.SKIP_STACK_FRAMES_BASE)
}
...
func (r *REFLECT) GetProperty(name string) (PROPERTY_INFO, bool) {
	...
	var field _reflect.StructField
	if field, found = r.objectType.FieldByName(name); found {
...
func __error(message interface{}) {
	logger.Logger.ErrorEx(message, MODULE_NAME_REFLECT, logger.SKIP_STACK_FRAMES_BASE)
}
