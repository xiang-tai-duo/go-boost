// Package boost
// File:        serve.go
// Url:         https://github.com/xiang-tai-duo/go-boost/blob/master/examples/serve.go
// Author:      Vibe Coding
...
func testServeMQTTServer() {
	fmt.Println("=== Example 8: MQTT Server ===")
	...
	err := server.Start()
	if err != nil {
		fmt.Printf("Failed to start MQTT server: %v\n", err)
		return
	}
	...
	if server.IsRunning() {
		...
		fmt.Println("✗ MQTT server is not running")
	}
	...
	fmt.Printf("Publishing message: Topic='%s', Payload='%s'\n", topic, payload)
	...
	err = server.Stop()
	if err != nil {
		fmt.Printf("Failed to stop MQTT server: %v\n", err)
	} else {
