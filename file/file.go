...
const (
	...
	FILE_PERMISSION      os.FileMode = 0644
	LARGE_FILE_THRESHOLD int64       = 100 * 1024 * 1024
	MODULE_NAME_FILE                 = "file"
)

//goland:noinspection GoUnusedFunction
func __debug(message string) {
	logger.Logger.DebugEx(message, MODULE_NAME_FILE, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __error(message interface{}) {
	logger.Logger.ErrorEx(message, MODULE_NAME_FILE, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __info(message string) {
	logger.Logger.InfoEx(message, MODULE_NAME_FILE, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __warning(message string) {
	logger.Logger.WarningEx(message, MODULE_NAME_FILE, logger.SKIP_STACK_FRAMES_BASE)
}
...
func Append(path string, data []byte) error {
	...
	if path != "" {
		var file *os.File
		if file, err = os.OpenFile(path, os.O_APPEND|os.O_WRONLY, FILE_PERMISSION); err == nil {
			defer file.Close()
	...
	if from != "" && to != "" {
		...
		if stat, err = os.Stat(from); err == nil {
			...
			if fileSize > LARGE_FILE_THRESHOLD {
				err = copyLargeFile(from, to, fileSize)
			} else {
				var data []byte
				if data, err = ReadAll(from); err == nil {
					err = os.WriteFile(to, data, FILE_PERMISSION)
				}
			}
		}
	} else {
...
func copyLargeFile(from string, to string, fileSize int64) error {
	...
	if src, err = os.Open(from); err == nil {
		...
		if dst, err = os.Create(to); err == nil {
			...
			if err == nil && bytesCopied != fileSize {
