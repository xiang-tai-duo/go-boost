...
func __debug(message string) {
	logger.Logger.DebugEx(message, MODULE_NAME_FONT, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __info(message string) {
	logger.Logger.InfoEx(message, MODULE_NAME_FONT, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnusedFunction
func __warning(message string) {
	logger.Logger.WarningEx(message, MODULE_NAME_FONT, logger.SKIP_STACK_FRAMES_BASE)
}

//goland:noinspection GoUnhandledErrorResult
func (f *FONT) ExportGlyphsToFile(fontName string) ([]rune, error) {
	result := make([]rune, 0)
	err := error(nil)
	...
	if fontInfo, err = f.getSystemFont(fontName); err == nil {
		...
		if file, err = os.OpenFile(fileName, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644); err == nil {
			...
			for i := 0; i < numberOfGlyphs; i++ {
				glyphIndex := sfnt.GlyphIndex(i)
				if character := f.getGlyphCharacter(fontInfo.Font, glyphIndex); character != "" {
					var displayCharacter rune
					if characterCode, err := strconv.ParseUint(fmt.Sprintf("0x%04X", []rune(character)[0])[2:], 16, 32); err == nil {
						displayCharacter = rune(characterCode)
						result = append(result, displayCharacter)
					} else {
						displayCharacter = rune(0)
...
func __error(message interface{}) {
	logger.Logger.ErrorEx(message, MODULE_NAME_FONT, logger.SKIP_STACK_FRAMES_BASE)
}
