// Package serve
// File:        mime.go
// Url:         https://github.com/xiang-tai-duo/go-boost/blob/master/serve/mime.go
// Author:      TRAE.AI
// Created:     2025/12/20 12:31:58
// Description: Provides mime type
// --------------------------------------------------------------------------------
package serve

import "strings"

//goland:noinspection GoSnakeCaseUsage
const (
	MIME_DEFAULT          = "application/octet-stream"
	MIME_TYPE_SEPARATOR   = "/"
	MIME_TYPE_X_PREFIX    = "x-"
	MIME_TYPE_PARTS_COUNT = 2
	FILE_EXTENSION_DOT    = "."
)

//goland:noinspection GoSnakeCaseUsage
var (
	EXTENSION_MAP = map[string]string{
		"text/html":                   ".html",
		"text/css":                    ".css",
		"application/javascript":      ".js",
		"application/json":            ".json",
		"application/xml":             ".xml",
		"text/plain":                  ".txt",
		"text/markdown":               ".md",
		"application/pdf":             ".pdf",
		"image/jpeg":                  ".jpg",
		"image/png":                   ".png",
		"image/gif":                   ".gif",
		"image/svg+xml":               ".svg",
		"image/x-icon":                ".ico",
		"image/bmp":                   ".bmp",
		"image/webp":                  ".webp",
		"audio/mpeg":                  ".mp3",
		"audio/wav":                   ".wav",
		"video/mp4":                   ".mp4",
		"video/webm":                  ".webm",
		"video/ogg":                   ".ogg",
		"application/zip":             ".zip",
		"application/x-tar":           ".tar",
		"application/gzip":            ".gz",
		"application/x-7z-compressed": ".7z",
		"application/vnd.rar":         ".rar",
		"application/msword":          ".doc",
		"application/vnd.openxmlformats-officedocument.wordprocessingml.document": ".docx",
		"application/vnd.ms-excel": ".xls",
		"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":         ".xlsx",
		"application/vnd.ms-powerpoint":                                             ".ppt",
		"application/vnd.openxmlformats-officedocument.presentationml.presentation": ".pptx",
		"text/csv":                      ".csv",
		"text/tab-separated-values":     ".tsv",
		"application/x-yaml":            ".yaml",
		"application/toml":              ".toml",
		"text/x-go":                     ".go",
		"text/x-java":                   ".java",
		"text/x-python":                 ".py",
		"text/x-php":                    ".php",
		"text/x-ruby":                   ".rb",
		"text/x-c":                      ".c",
		"text/x-c++":                    ".cpp",
		"text/x-csharp":                 ".cs",
		"text/x-swift":                  ".swift",
		"text/x-kotlin":                 ".kt",
		"text/x-rust":                   ".rs",
		"text/typescript":               ".ts",
		"text/jsx":                      ".jsx",
		"text/tsx":                      ".tsx",
		"text/x-vue":                    ".vue",
		"text/x-svelte":                 ".svelte",
		"application/wasm":              ".wasm",
		"font/ttf":                      ".ttf",
		"font/otf":                      ".otf",
		"font/woff":                     ".woff",
		"font/woff2":                    ".woff2",
		"application/vnd.ms-fontobject": ".eot",
		"application/sql":               ".sql",
		"application/x-sh":              ".sh",
		"application/x-msdos-program":   ".bat",
		"text/x-powershell":             ".ps1",
		"application/x-msdownload":      ".exe",
		"application/x-sharedlib":       ".so",
		"application/x-mach-binary":     ".dylib",
		"application/vnd.android.package-archive": ".apk",
		"application/x-itunes-ipa":                ".ipa",
		"application/x-deb":                       ".deb",
		"application/x-rpm":                       ".rpm",
		"application/x-iso9660-image":             ".iso",
		"application/octet-stream":                ".bin",
	}

	MIME_TYPES = map[string]string{
		".html":   "text/html",
		".htm":    "text/html",
		".css":    "text/css",
		".js":     "application/javascript",
		".json":   "application/json",
		".xml":    "application/xml",
		".txt":    "text/plain",
		".md":     "text/markdown",
		".pdf":    "application/pdf",
		".jpg":    "image/jpeg",
		".jpeg":   "image/jpeg",
		".png":    "image/png",
		".gif":    "image/gif",
		".svg":    "image/svg+xml",
		".ico":    "image/x-icon",
		".bmp":    "image/bmp",
		".webp":   "image/webp",
		".mp3":    "audio/mpeg",
		".wav":    "audio/wav",
		".mp4":    "video/mp4",
		".webm":   "video/webm",
		".ogg":    "video/ogg",
		".zip":    "application/zip",
		".tar":    "application/x-tar",
		".gz":     "application/gzip",
		".7z":     "application/x-7z-compressed",
		".rar":    "application/vnd.rar",
		".doc":    "application/msword",
		".docx":   "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
		".xls":    "application/vnd.ms-excel",
		".xlsx":   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
		".ppt":    "application/vnd.ms-powerpoint",
		".pptx":   "application/vnd.openxmlformats-officedocument.presentationml.presentation",
		".csv":    "text/csv",
		".tsv":    "text/tab-separated-values",
		".yaml":   "application/x-yaml",
		".yml":    "application/x-yaml",
		".toml":   "application/toml",
		".ini":    "text/plain",
		".conf":   "text/plain",
		".log":    "text/plain",
		".go":     "text/x-go",
		".java":   "text/x-java",
		".py":     "text/x-python",
		".php":    "text/x-php",
		".rb":     "text/x-ruby",
		".c":      "text/x-c",
		".cpp":    "text/x-c++",
		".h":      "text/x-c",
		".hpp":    "text/x-c++",
		".cs":     "text/x-csharp",
		".swift":  "text/x-swift",
		".kt":     "text/x-kotlin",
		".rs":     "text/x-rust",
		".ts":     "text/typescript",
		".jsx":    "text/jsx",
		".tsx":    "text/tsx",
		".vue":    "text/x-vue",
		".svelte": "text/x-svelte",
		".wasm":   "application/wasm",
		".ttf":    "font/ttf",
		".otf":    "font/otf",
		".woff":   "font/woff",
		".woff2":  "font/woff2",
		".eot":    "application/vnd.ms-fontobject",
		".map":    "application/json",
		".sql":    "application/sql",
		".sh":     "application/x-sh",
		".bat":    "application/x-msdos-program",
		".ps1":    "text/x-powershell",
		".exe":    "application/x-msdownload",
		".dll":    "application/x-msdownload",
		".so":     "application/x-sharedlib",
		".dylib":  "application/x-mach-binary",
		".apk":    "application/vnd.android.package-archive",
		".ipa":    "application/x-itunes-ipa",
		".deb":    "application/x-deb",
		".rpm":    "application/x-rpm",
		".iso":    "application/x-iso9660-image",
		".img":    "application/octet-stream",
		".bin":    "application/octet-stream",
		".dat":    "application/octet-stream",
	}
)

//goland:noinspection GoUnusedExportedFunction
func GetFileExtensionName(mimeType string) string {
	result := ""
	if fileExtensionName, ok := EXTENSION_MAP[strings.ToLower(mimeType)]; ok {
		result = fileExtensionName
	} else {
		parts := strings.Split(mimeType, MIME_TYPE_SEPARATOR)
		if len(parts) == MIME_TYPE_PARTS_COUNT {
			subtype := parts[1]
			if strings.HasPrefix(subtype, MIME_TYPE_X_PREFIX) {
				subtype = strings.TrimPrefix(subtype, MIME_TYPE_X_PREFIX)
			}
			result = FILE_EXTENSION_DOT + subtype
		}
	}
	return result
}

//goland:noinspection GoUnusedExportedFunction
func GetMimeType(fileExtensionName string) string {
	result := MIME_DEFAULT
	if mimeType, ok := MIME_TYPES[strings.ToLower(fileExtensionName)]; ok {
		result = mimeType
	}
	return result
}
