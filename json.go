// Package boost
// File:        json.go
// Url:         https://github.com/xiang-tai-duo/go-boost/blob/master/json.go
// Author:      Vibe Coding
...
func (j *JSON) Format(indent string) (string, error) {
	...
	var result string
	if err = json.Unmarshal([]byte(j.text), &parsedValue); err == nil {
...
func NewJSON(initialValue ...interface{}) *JSON {
	...
	if len(initialValue) > 0 {
		...
		if s, ok := initialValue[0].(string); ok {
			var parsedValue interface{}
			if err := json.Unmarshal([]byte(s), &parsedValue); err == nil {
				j.text = s
...
func (j *JSON) Unmarshal(target interface{}) error {
	return json.Unmarshal([]byte(j.text), target)
}
...
func (j *JSON) WriteFile(filePath string, indent ...string) error {
	...
	if indentValue != "" {
		if jsonBytes, err = json.MarshalIndent(j.value, "", indentValue); err == nil {
			// No return here, continue
		} else {
			return err
		}
	} else {
		if jsonBytes, err = json.Marshal(j.value); err == nil {
			// No return here, continue
		} else {
			return err
		}
